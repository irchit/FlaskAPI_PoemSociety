...
def predict_cb_views(user, poems_unseen):
    ...
    for poem in poems_unseen:
        ...
        # Preferință limbă
        lang_score = lang_prefs.get(lang, 0.01)
...
def hybrid_recommendations(user_activ, users, views, poems, following_list):
    ...
    # 3. Calculăm scoruri agregate pentru fiecare poem nevăzut (CF)
    ...
    # 5. Aplicăm content-based filtering
    predicted = predict_cb_views(user_activ, selected_poems)

    # 6. Pregătim scorurile CF pentru toate poeme selectate
    cf_score_dict = defaultdict(float)
    for pid, score in poem_scores.items():
        cf_score_dict[pid] = score
    for pid in selected_poem_ids:
        if pid not in cf_score_dict:
            cf_score_dict[pid] = 0.0

    # 7. Combinăm scorurile CF și CB într-un scor final, dar punem scor = 0 dacă e deja văzut
    alpha = 0.6
    recomandari = []
    for pred in predicted:
        poem_id = pred["poem_id"]
        cb = pred["cb_score"]
        cf = cf_score_dict.get(poem_id, 0.0)
        final_score = alpha * cb + (1 - alpha) * cf

        # ⚠️ Dacă poemul a fost deja văzut, scorul trebuie să fie 0
        if poem_id in seen:
            final_score = 0.0

        recomandari.append({
            "poem": poems_dict[poem_id],
            "score": final_score
        })

    # 8. Sortăm după scorul final
    recomandari.sort(key=lambda x: x["score"], reverse=True)
